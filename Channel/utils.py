...
def get_channel(model_name="conv3", feat_dim=128, dataset="CIFAR10", out_norm = True):
    if "conv" in model_name:
        if dataset in ["CIFAR10", "CIFAR100"]:
            ...
            size = 64
        elif dataset in ["ImageNette", "imagenet-woof", "imagenet-100"]:
            size = 128
        else:
            size = 224
        model = ConvNet(
...
if __name__ == '__main__':
    model = get_channel(model_name="resnet18_modified", dataset="CIFAR10", feat_dim=128)
    # model = get_channel(model_name="resnet101_modified", dataset="CIFAR10", feat_dim=128)
    # model = get_channel(model_name="conv3", dataset="CIFAR10", feat_dim=128)

    # model = get_channel(model_name="resnet18_modified", dataset="CIFAR100", feat_dim=128)
    # model = get_channel(model_name="resnet101_modified", dataset="CIFAR100", feat_dim=128)
    # model = get_channel(model_name="conv3", dataset="CIFAR100", feat_dim=128)
